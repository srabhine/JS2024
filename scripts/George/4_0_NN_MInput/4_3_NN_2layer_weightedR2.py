...
def create_model(input_dimensions1, input_dimensions2, lr, weight_decay):
    ...
    layer2 = Dense(1, activation='tanh', name="responder_6_return")(layer2)
    ...
    layer1 = Dense(256, activation='swish', kernel_regularizer=regularizers.l2(1e-4))(layer1)
    layer1 = Dropout(0.1)(layer1)
    ...
    # Create the model using tf.keras.Model, not CustomModel
    ...
    model.compile(
        ...
        loss={
            "responder_6": custom_weighted_r2_loss,
            "responder_6_return": "mse"
        },
        metrics={
            "responder_6": tf.keras.metrics.R2Score(),
...
w_train = np.asarray(w_train).reshape(-1, 1)
y_train1_with_weights = np.concatenate([y_train1, w_train], axis=1)

y_valid1 = np.asarray(y_valid1).reshape(-1, 1)
w_valid = np.asarray(w_valid).reshape(-1, 1)
y_valid1_with_weights = np.concatenate([y_valid1, w_valid], axis=1)
...
model.fit(
    x={"input_1": X_train_1, "input_2": X_train_2},
    y={
        "responder_6": y_train1_with_weights,
        "responder_6_return": y_train2
    },
    validation_data=(
        {"input_1": X_valid_1, "input_2": X_valid_2},
        {
            "responder_6": y_valid1_with_weights,
            "responder_6_return": y_valid2
        }
    ),
    batch_size=8029,
    epochs=100,
