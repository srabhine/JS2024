...
def create_model(input_dimensions, lr):
    ...
    layer2 = Dense(64, activation='swish')(layer2)

    input_to_ly1 = Concatenate()([layer2, input_group_1])

    layer1 = Dense(128, activation='swish')(input_to_ly1)
    layer1 = Dense(64, activation='swish')(layer1)

    output = Concatenate()([layer1, layer2]) # output 6 returns

    # Create the model
    model = Model(inputs=[input_group_1], outputs=output)

    # Compile the model
    model.compile(optimizer=optimizers.Adam(learning_rate=lr), loss=['mse','mse'], metrics=[tf.keras.metrics.R2Score(), tf.keras.metrics.R2Score()])
