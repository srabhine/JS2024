...
data_path = "/home/zt/pyProjects/JaneSt/Team/data/mean_data/train.parquet"
model_saving_path = "/home/zt/pyProjects/JaneSt/Team/scripts/George/models/3_base_model_mean_data"
model_saving_name = "model_3_Base_mean_data_{epoch:02d}.keras"

feature_names = [f"mean_feature_{i:02d}" for i in range(79)]
label_name = 'mean_responder_6'
weight_name = 'weight'

data_train = pl.scan_parquet(data_path).select(
    pl.int_range(pl.len(), dtype=pl.UInt32).alias("id"),
    pl.all(),).filter(
    pl.col("date_id").le(1500))
data_train = data_train.collect().to_pandas()
data_train = data_train.fillna(0)

data_valid = pl.scan_parquet(data_path).select(
    pl.int_range(pl.len(), dtype=pl.UInt32).alias("id"),
    pl.all(),).filter(
    pl.col("date_id").gt(1501))
data_valid = data_valid.collect().to_pandas()
