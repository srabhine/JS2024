...
df_by_sym = stack_features_by_sym(df).fillna(0)

# feats_last, feats_new = [], []
# for i in range(len(df_by_sym)):
#     if i < 1:
#         continue
#     if df_by_sym.index[i][0] != df_by_sym.index[i-1][0]:
#         feats_last.append(df_by_sym.iloc[i-1])
#         feats_new.append(df_by_sym.iloc[i])
#
# feats_last = pd.concat(feats_last, axis=1)
# feats_new = pd.concat(feats_new, axis=1)
#
# sym = 1
# feats_last.loc[(TARGET, sym)]
# feats_new.loc[(TARGET, sym)]
#
# np_comp = np.column_stack(
#     (feats_last.loc[(TARGET, sym)].values,
#                      feats_new.loc[(TARGET, sym)].values))
# np_comp = np.sign(np_comp)
# np_diff = np_comp[:, 1] - np_comp[:, 0]
# len(np_diff[np_diff==0]) / len(np_diff)
...
for sym in SYMBOLS:
    ...
    for i in range(len(y)):
        if i < 1:
            y_pred_sign.iloc[i] = 1
            continue
        if df_by_sym.index[i][0] != df_by_sym.index[i - 1][0]:
            y_pred_sign.iloc[i-1] = y_sign.iloc[i-1]
        if y_feat.iloc[i] == y_feat.iloc[i-1]:
            # y_pred_sign.iloc[i] = y_sign.iloc[i-1]
            y_pred_sign.iloc[i] = y_pred_sign.iloc[i-1]
        else:
            # y_pred_sign.iloc[i] = -y_sign.iloc[i-1]
            y_pred_sign.iloc[i] = -y_pred_sign.iloc[i-1]
        y_pred.iloc[i] = 0.1 * sg_y * y_pred_sign.iloc[i]
        # y_pred.iloc[i] = 0.025 * sg_y * y_pred_sign.iloc[i]
        # y_pred.iloc[i] = sg_y * y_pred_sign.iloc[i]
